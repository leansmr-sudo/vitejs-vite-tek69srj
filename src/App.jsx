    ...
    <div style={S.loginRoot}>
      <div style={S.loginCard}>
        ...
        <div style={S.loginLogo}>
          ...
          <div style={S.loginTitle}>SMRHC<span style={S.loginAccent}> COACH</span></div>
...
export default function App() {
  ...
  const [statsEquipo, setStatsEquipo]   = useState("Superior");
  const [statsAnio, setStatsAnio]       = useState(new Date().getFullYear().toString());
      ...
      <div style={S.page}>
        ...
        {matches.map((m,i) => (
          <div key={m.firebaseId||i} style={S.histCard}>
            ...
            {m.savedBy && <div style={{fontSize:11, color:"#3a5a8a", marginTop:4}}>Guardado por: {m.savedBy}</div>}
            ...
            {requiereJugador && (
              <Field label={<span>Jugador <span style={{color:"#ff6b6b"}}>*</span></span>}>
                {match.players.filter(p => p.name).length === 0
                  ...
                  : <select style={{...S.input, borderColor: !logForm.jugador ? "#ff6b6b88" : "#1a2244"}} value={logForm.jugador} onChange={e => setLogForm(f=>({...f, jugador: e.target.value}))}>
          ...
          {match.log.length > 0 && (
            <div style={{marginTop:20}}>
              ...
              {[...match.log].reverse().map(e => (
                <div key={e.id} style={{...S.logRow, borderLeft:`3px solid ${e.equipo==="Propio"?"#2979d4":"#ff6b6b"}`}}>
                  <span style={{flex:.4,color:"#888",fontSize:12}}>{e.minuto||"—"}</span>
                  <span style={{flex:.4}}><span style={S.badge}>{e.tiempo}</span></span>
                  <span style={{flex:.8,color:e.equipo==="Propio"?"#2979d4":"#ff6b6b",fontSize:12,fontWeight:600}}>{e.equipo}</span>
        ...
        <div style={S.page}>
          ...
          <div style={{...S.summCard,marginTop:16}}>
            ...
            <div style={S.summHead}>
              ...
              <span style={{flex:.6,textAlign:"center",color:"#2979d4"}}>Propio</span>
            ...
            {activeSummary.map((row,i) => {
              ...
              return (
                <div key={i} style={{...S.summRow,...(i%2===0?S.summRowAlt:{})}}>
                  ...
                  <span style={{flex:.6,textAlign:"center",fontWeight:700,color:"#2979d4"}}>{row.propio}</span>
                  <span style={{flex:1.6}}>
                    {total > 0 ? (
                      <div style={S.barWrap}>
                        <div style={{...S.barFill, width:`${propPct}%`, background: row.type==="neg"?"#ff6b6b":row.type==="pos"?"#2979d4":"#888"}}/>
    ...
    <header style={S.header}>
      <div style={S.headerInner}>
        <div style={S.logo}>
          ...
          <span style={S.logoTxt}>SMRHC<span style={S.logoAcc}> COACH</span></span>
        </div>
...
function Field({ label, children, style }) {
  return (
    <label style={{display:"flex",flexDirection:"column",gap:6,fontSize:11,color:"#7a8aaa",textTransform:"uppercase",letterSpacing:1,...style}}>
      {label}{children}
    </label>
  );
}
...
const S = {
  ...
  loginRoot: { minHeight:"100vh", background:"#0a0c12", display:"flex", alignItems:"center", justifyContent:"center", padding:16 },
  loginCard: { background:"#0d1120", border:"1px solid #1e3a1e", borderRadius:16, padding:"40px 32px", width:"100%", maxWidth:380 },
  ...
  loginAccent: { color:"#2979d4" },
  loginSubtitle: { fontSize:12, color:"#4a6a9a", letterSpacing:2, textTransform:"uppercase" },
  loginForm: { display:"flex", flexDirection:"column", gap:16 },
  loginLabel: { display:"flex", flexDirection:"column", gap:6, fontSize:11, color:"#7a8aaa", textTransform:"uppercase", letterSpacing:1 },
  loginInput: { background:"#0c0f1a", border:"1px solid #1e3a1e", borderRadius:8, padding:"12px 14px", color:"#e8f0e8", fontSize:14, fontFamily:"inherit", outline:"none" },
  loginBtn: { background:"#2979d4", border:"none", color:"#0a0c12", borderRadius:10, padding:"14px", fontSize:15, fontWeight:"bold", cursor:"pointer", fontFamily:"inherit", marginTop:8 },
  ...
  pillGreen: { background:"#2979d4", border:"none", color:"#0a0c12", borderRadius:20, padding:"5px 14px", cursor:"pointer", fontWeight:"bold", fontSize:12, fontFamily:"inherit" },
  ...
  editingBanner: { maxWidth:860, margin:"8px auto 0", padding:"8px 16px", background:"#0a1228", border:"1px solid #4a7a1a", borderRadius:8, fontSize:12, color:"#aaccff", textAlign:"center" },
  ...
  scoreLabel: { fontSize:11, color:"#6a8aaa", textTransform:"uppercase", letterSpacing:1.5, textAlign:"center" },
  scoreCtrl: { display:"flex", alignItems:"center", gap:10 },
  scoreBtn: { background:"#0f1428", border:"1px solid #2979d4", color:"#2979d4", borderRadius:8, width:34, height:34, cursor:"pointer", fontSize:20, lineHeight:1 },
  scoreNum: { fontSize:52, fontWeight:"bold", color:"#fff", minWidth:64, textAlign:"center", lineHeight:1 },
  scoreInput: { fontSize:46, fontWeight:"bold", color:"#fff", width:72, textAlign:"center", background:"transparent", border:"none", borderBottom:"2px solid #2979d480", outline:"none", fontFamily:"'Georgia','Times New Roman',serif", MozAppearance:"textfield" },
  scoreCenter: { display:"flex", flexDirection:"column", alignItems:"center", gap:2, paddingBottom:4 },
  scoreVS: { fontSize:13, color:"#2979d4", fontWeight:"bold", letterSpacing:3 },
  scoreInfo: { fontSize:11, color:"#4a6a9a" },
  scoreInfo2: { fontSize:12, color:"#7aaa7a", fontWeight:"bold" },
  nav: { maxWidth:860, margin:"16px auto 0", padding:"4px", display:"flex", gap:4, background:"#0e1220", borderRadius:12 },
  navBtn: { flex:1, padding:"9px 4px", background:"transparent", border:"none", color:"#6a8aaa", borderRadius:9, cursor:"pointer", fontFamily:"inherit", fontSize:12, transition:"all .15s" },
  navBtnActive: { background:"#2979d4", color:"#0a0c12", fontWeight:"bold" },
  page: { maxWidth:860, margin:"16px auto 40px", padding:"0 16px" },
  pageTitle: { fontSize:13, color:"#2979d4", textTransform:"uppercase", letterSpacing:3, marginBottom:16, fontWeight:"bold" },
  grid2: { display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 },
  input: { background:"#0c0f1a", border:"1px solid #1e3a1e", borderRadius:8, padding:"10px 12px", color:"#e8f0e8", fontSize:13, fontFamily:"inherit", outline:"none" },
  logForm: { background:"#0d1120", border:"1px solid #1e3a1e", borderRadius:14, padding:"16px" },
  logFormRow: { display:"flex", gap:10, alignItems:"flex-end", marginBottom:12 },
  segCtrl: { display:"flex", gap:3, flexWrap:"wrap" },
  segBtn: { background:"#0f1428", border:"1px solid #1e3a1e", color:"#7a9aca", borderRadius:6, padding:"8px 12px", cursor:"pointer", fontFamily:"inherit", fontSize:12, whiteSpace:"nowrap" },
  segBtnActive: { background:"#2979d4", color:"#0a0c12", border:"1px solid #2979d4", fontWeight:"bold" },
  accionGrid: { display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6 },
  accionBtn: { background:"#0f1428", border:"1px solid #1e3a1e", color:"#7a9aca", borderRadius:8, padding:"10px 4px", cursor:"pointer", fontFamily:"inherit", fontSize:12, display:"flex", flexDirection:"column", alignItems:"center", gap:2 },
  accionBtnActive: { background:"#0d1a3a", border:"1px solid #2979d4", color:"#2979d4" },
  addBtn: { width:"100%", background:"#2979d4", border:"none", color:"#0a0c12", borderRadius:10, padding:"13px", fontSize:14, fontWeight:"bold", cursor:"pointer", fontFamily:"inherit", marginTop:12 },
  addBtnDisabled: { background:"#141828", color:"#4a6a9a", cursor:"not-allowed" },
  jugadorWarning: { background:"#2a1a0e", border:"1px solid #ff6b6b55", borderRadius:8, padding:"10px 12px", fontSize:12, color:"#ff9a6b" },
  logHeader: { display:"flex", padding:"8px 12px", fontSize:11, color:"#4a6a9a", textTransform:"uppercase", letterSpacing:1, borderBottom:"1px solid #1e3a1e" },
  logRow: { display:"flex", alignItems:"center", padding:"10px 12px", borderBottom:"1px solid #131f13", fontSize:13 },
  badge: { background:"#141828", color:"#7aaa7a", fontSize:10, padding:"2px 6px", borderRadius:4, fontWeight:"bold" },
  ...
  playerRow: { display:"flex", alignItems:"center", gap:10, background:"#0d1120", border:"1px solid #1a2a1a", borderRadius:10, padding:"10px 14px", cursor:"pointer" },
  playerNumBadge: { width:28, height:28, background:"#141828", borderRadius:6, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:"bold", color:"#2979d4", flexShrink:0 },
  ...
  playerPosLabel: { fontSize:11, color:"#4a6a9a" },
  playerPtsBadge: { fontSize:12, color:"#f5c842", fontWeight:"bold" },
  playerArrow: { color:"#3a5a8a", fontSize:18, marginLeft:4 },
  pedHeader: { display:"flex", alignItems:"center", gap:12, background:"#0d1120", border:"1px solid #1a2a1a", borderRadius:12, padding:"14px 16px", marginBottom:16 },
  backBtn: { background:"transparent", border:"none", color:"#2979d4", cursor:"pointer", fontSize:13, fontFamily:"inherit", whiteSpace:"nowrap" },
  pedName: { fontSize:17, fontWeight:"bold" },
  pedPos: { fontSize:11, color:"#4a6a9a" },
  pedPts: { marginLeft:"auto", fontSize:30, fontWeight:"bold", color:"#f5c842" },
  statsGrid: { display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 },
  statCard: { background:"#0d1120", border:"1px solid #1a2a1a", borderRadius:10, padding:"12px" },
  statIcon: { fontSize:18, marginBottom:4 },
  statLbl: { fontSize:11, color:"#6a8aaa", marginBottom:8 },
  statPtsHint: { color:"#f5c842" },
  statCtrl: { display:"flex", alignItems:"center", gap:8, justifyContent:"center" },
  statBtn: { background:"#141828", border:"1px solid #2a4a2a", color:"#2979d4", borderRadius:6, width:32, height:32, cursor:"pointer", fontSize:16 },
  ...
  pedNavBtn: { background:"#0d1120", border:"1px solid #1e3a1e", color:"#2979d4", borderRadius:8, padding:"9px 18px", cursor:"pointer", fontFamily:"inherit", fontSize:12 },
  resScorebig: { display:"flex", alignItems:"center", justifyContent:"center", gap:20, marginBottom:20, background:"#0d1120", borderRadius:14, padding:"20px" },
  resTeam: { flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:6 },
  resTeamName: { fontSize:11, color:"#6a8aaa", textTransform:"uppercase", letterSpacing:1 },
  resScoreNum: { fontSize:52, fontWeight:"bold", color:"#2979d4" },
  resMid: { display:"flex", flexDirection:"column", alignItems:"center", gap:4 },
  resDash: { fontSize:32, color:"#1e3060" },
  resRivalName: { fontSize:12, color:"#6a8aaa" },
  summCard: { background:"#0d1120", border:"1px solid #1a2a1a", borderRadius:12, overflow:"hidden" },
  summCardTitle: { padding:"12px 16px", fontSize:11, color:"#2979d4", textTransform:"uppercase", letterSpacing:2, borderBottom:"1px solid #1a2a1a", fontWeight:"bold" },
  summHead: { display:"flex", padding:"8px 16px", fontSize:10, color:"#4a6a9a", textTransform:"uppercase", letterSpacing:.5, borderBottom:"1px solid #1a2a1a" },
  summRow: { display:"flex", alignItems:"center", padding:"9px 16px", fontSize:13, borderBottom:"1px solid #111811" },
  summRowAlt: { background:"#0c0f1a" },
  barWrap: { height:16, background:"#0e1220", borderRadius:8, overflow:"hidden", position:"relative", flex:1, margin:"0 4px" },
  ...
  notesBox: { background:"#0d1120", border:"1px solid #1a2a1a", borderRadius:8, padding:"12px 14px", fontSize:13, color:"#aaa", margin:"12px 0" },
  exportBtn: { width:"100%", background:"transparent", border:"1px solid #2979d4", color:"#2979d4", borderRadius:10, padding:"12px", fontSize:14, fontWeight:"bold", cursor:"pointer", fontFamily:"inherit", marginTop:8, marginBottom:6 },
  saveBtn: { width:"100%", background:"#2979d4", border:"none", color:"#0a0c12", borderRadius:10, padding:"14px", fontSize:15, fontWeight:"bold", cursor:"pointer", fontFamily:"inherit", marginTop:8 },
  empty: { textAlign:"center", color:"#3a5a8a", padding:"32px 16px", fontSize:14 },
  histCard: { background:"#0d1120", border:"1px solid #1a2a1a", borderRadius:12, padding:"16px", marginBottom:10 },
  histTop: { display:"flex", justifyContent:"space-between", marginBottom:8 },
  histRival: { fontSize:15, fontWeight:"bold" },
  histDate: { fontSize:12, color:"#4a6a9a" },
  histScoreRow: { display:"flex", alignItems:"center", gap:12, marginBottom:6 },
  histScore: { fontSize:32, fontWeight:"bold", color:"#2979d4" },
  histDash: { fontSize:20, color:"#1e3060" },
  histMeta: { fontSize:12, color:"#4a6a9a" },
  histActions: { display:"flex", gap:8, marginTop:12 },
  histBtnEdit: { flex:1, background:"#141828", border:"1px solid #2a4a2a", color:"#2979d4", borderRadius:8, padding:"9px", cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:"bold" },
  ...
  histBtnCancel: { background:"#141828", border:"1px solid #2a4a2a", color:"#aaa", borderRadius:8, padding:"8px 16px", cursor:"pointer", fontFamily:"inherit", fontSize:13 },
};